...
struct LoginView: View {
    ...
    @StateObject private var firebaseWebViewController = FirebaseWebViewController()
    ...
    func fetchUserData() {
        ...
        db.collection("users").document(userID).getDocument { snapshot, error in
            if let error = error {
                ...
                // Update properties of userData directly
                userData.name = data["name"] as? String ?? "Unknown Name"
                userData.email = data["email"] as? String ?? ""
                userData.phoneNumber = data["phoneNumber"] as? String ?? ""
                userData.userType = data["userType"] as? String ?? ""
                userData.caretakerID = data["caretakerID"] as? String
            } else {
                // Handle case: document doesn't exist
            }
        }
    }
   /* private mutating func createFirebaseWebViewController() { // Helper function
          self.firebaseWebViewController = FirebaseWebViewController()
      }*/
